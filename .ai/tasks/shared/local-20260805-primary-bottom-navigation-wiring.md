# Primary Bottom Navigation Wiring

Status: Ready for Review
Objective: Wire every visible production primary bottom-navigation item to its typed `MainRoute` while preserving canonical routes and selected-state behavior.
Branch: `fix/primary-bottom-navigation-wiring`
Scope: `shared/ui` navigation shell and affected screen route boundaries, focused navigation tests, and navigation validation documentation
Created: `2026-08-05`
Last Updated: `2026-08-05`
Issue: `https://github.com/im-tnyx/Tio-Ledger/issues/34`
Pull Request: `https://github.com/im-tnyx/Tio-Ledger/pull/36`

## Required Context

- `.ai/core/architecture.md`
- `.ai/core/coding-rules.md`
- `.ai/core/ui-rules.md`
- `.ai/core/workflow-rules.md`
- `AGENTS.md`
- `README.md`
- `docs/README.md`
- `docs/engineering-guidelines.md`
- `docs/definition-of-done.md`
- `docs/status/NAVIGATION_VALIDATION_REPORT.md`
- `.github/PUSH_TEMPLATE.md`

## Constraints

- Keep `TioNavigationGraphs.main.bottomNavigationRoutes` as the canonical route source.
- Preserve the five approved primary destinations: Dashboard, Accounts, Transactions, Categories, and Budgets.
- Do not redesign Dashboard content or introduce a new screen hierarchy.
- Keep Reports and Loans navigable to primary tabs without falsely selecting a primary item.
- Preserve existing detail, add, SMS-review, and back-navigation flows.
- No financial, persistence, schema, analytics, loan, notification, or automation changes.
- Reuse existing `TioBottomNavigation` and typed `MainRoute` contracts.

## Implementation Plan

- [x] Verify issue #34 against current `main` runtime source.
- [x] Complete Loan Payoff Analytics post-merge closeout.
- [x] Create dedicated branch from updated `main`.
- [x] Introduce a shared, testable bottom-navigation model derived from `MainGraph.bottomNavigationRoutes`.
- [x] Wire `AccountsRoute` and `AccountsScreen` through typed navigation callbacks.
- [x] Wire Dashboard/placeholder bottom-navigation taps through `RootNavigationHost`.
- [x] Preserve existing working screen callbacks without speculative refactoring.
- [x] Add focused route-order, selected-state, unknown-item, and callback-forwarding tests.
- [x] Update navigation validation documentation.
- [x] Run implementation-head CI and review the implementation diff.
- [x] Open draft PR #36.
- [ ] Run final documentation-head CI and complete final PR review.
- [ ] Merge PR #36 and perform post-merge task archival.

## Validation

Implementation head `5f059f97628674de399857ddfdf616b325832de0` passed GitHub Actions CI run #345:

- Shared metadata compilation.
- Critical tests, including `MainBottomNavigationModelTest`.
- SQLDelight migration verification.
- `ktlintCheck`.
- `detekt`.

The implementation-introduced constructor warning was removed. Remaining compiler warnings are pre-existing and outside this task scope.

## Changed Areas

- `shared/ui/src/commonMain/kotlin/com/tioledger/ui/navigation/MainBottomNavigation.kt`
- `shared/ui/src/commonMain/kotlin/com/tioledger/ui/accounts/AccountsScreen.kt`
- `shared/ui/src/commonMain/kotlin/com/tioledger/ui/accounts/AccountsPreviews.kt`
- `shared/ui/src/commonMain/kotlin/com/tioledger/ui/shell/RootNavigationHost.kt`
- `shared/ui/src/commonTest/kotlin/com/tioledger/ui/navigation/MainBottomNavigationModelTest.kt`
- `docs/status/NAVIGATION_VALIDATION_REPORT.md`
- `.ai/current.md`
- This task file

## Next Action

Run exact-head CI for the documentation-complete branch, perform the final scope and review-thread audit, then merge PR #36 if all checks remain green.
